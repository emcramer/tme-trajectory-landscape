...
def _():
    ...
    # Define paths. Anchor to the repo root (3 levels above this script:
    # <repo>/tme-trajectory-landscape/figures/figure_3/<this file>) so the
    # notebook works regardless of CWD when invoked from the CLI.
    ...
    # Save outputs alongside the script in <figure_3>/output/, matching the
    # figure_2 / panel_c convention.
...
def _(mo):
    mo.md(r"""
    # Wang ROI Selection Visualization
    """)
    return
...
def _(df_linked, mo):
    ...
    phase_selector = mo.ui.dropdown(
        options=phases,
        value="Post-treatment", #phases[0] if phases else None,
...
def _(df_linked, mo, phase_selector):
    ...
    image_selector = mo.ui.dropdown(
        ...
        value="NT010 - NTImg0565", #list(image_options.keys())[0] if image_options else None,
...
def _(fig, output_dir):
    output_dir.mkdir(parents=True, exist_ok=True)
    png_path = output_dir / "figure_3_panel_b_imc.png"
    svg_path = output_dir / "figure_3_panel_b_imc.svg"
