...
##########
# NOTE: This notebook has been modified to have the options removed and the default values set to the ones used in the final figure. The original code with the dropdown options is available on request (e.g. enabling all other TME states as potential desireable or undesirable targets). The rest of the code is unchanged. Other modifications to this file include changing output and input directories to match the git repo and CodeOCean capsule directory structure.
##########
...
def _(pd):
    # Load Data
    data_dir = "data/"
    state_labels_file = f"{data_dir}/abm_windows_clustered_with_state_label.csv"
...
def _(mo, param_options):
    ...
    intervention_param_select = mo.ui.dropdown(
        ...
        value="transformation_rate[@name='exhausted_T_cell']",
        label="Intervention Parameter"
    )

    intervention_range = mo.ui.range_slider(
        start=0, stop=100, step=1, value=[0, 16],
...
def _(
    ...
    print(f"Selected Intervention Parameter: {intervention_param_select.value}")
    print(f"Intervention Percentile Range: {intervention_range.value}")
    ...
    print(f"**Intervention Dynamics defined using {i_sim_count} simulations.** (`{clean_param_name(i_col)}`: {i_low_val:.5f} - {i_high_val:.5f})")
...
def _(
    ...
    #fname = f"intercept_trajectory_{p_name}_{p_min}_{p_max}_figure_6_panel_e.png"
    fname = f"figure_6_panel_e.png"
    ...
    #fname_svg = f"intercept_trajectory_{p_name}_{p_min}_{p_max}_figure_6_panel_e.svg"
    fname_svg = f"figure_6_panel_e.svg"
...
def _(max_time_idx, mo):
    ...
    intervention_time_slider = mo.ui.slider(
        start=0, stop=max_time_idx, step=1, value=100,
...
def _(
    ...
    #fname_bar = f"outcome_comparison_{p_name_bar}_{p_min_bar}_{p_max_bar}_t{t_int}_figure_6_panel_f.png"
    fname_bar = f"figure_6_panel_f.png"
    fig_bar.savefig(f"{output_dir}/{fname_bar}", dpi=300, bbox_inches='tight')

    #fname_bar_svg = f"outcome_comparison_{p_name_bar}_{p_min_bar}_{p_max_bar}_t{t_int}_figure_6_panel_f.svg"
    fname_bar_svg = f"figure_6_panel_f.svg"
